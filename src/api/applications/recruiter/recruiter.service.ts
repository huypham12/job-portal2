...
export class RecruiterApplicationService {
  ...
  async updateApplicationStatus(recruiterId: string, applicationId: string, data: UpdateStatusDTO['body']) {
    ...
    // If rejecting, update rejected_at and rejected_reason
    if (status === application_status.rejected) {
      updateData.rejected_at = new Date()
      updateData.rejected_reason = reason || 'Application rejected by recruiter'
    }
   ...
   */
  async sendOffer(recruiterId: string, applicationId: string, data: SendOfferDTO['body']) {
    // Verify access
    const app = await this.verifyApplicationAccess(applicationId, recruiterId)

    // Check if application is in a valid state to receive offer
    const currentApp = await prisma.applications.findUnique({
      where: { id: applicationId },
      select: {
        status: true,
    ...
    if (currentApp.offer_sent_at) {
      throw new HttpError('Offer has already been sent for this application', HTTP_STATUS.BAD_REQUEST)
    }

    // Check if application status allows sending offer
    if (currentApp.status === 'rejected' || currentApp.status === 'withdrawn') {
      throw new HttpError('Cannot send offer to a rejected or withdrawn application', HTTP_STATUS.BAD_REQUEST)
    }

    // Build offer metadata
    const offerMetadata: any = {
      ...(data.metadata || {}),
      offer_details: {
        salary: data.salary,
    ...
    return {
      id: updatedApplication.id,
      status: updatedApplication.status,
      offer_sent_at: updatedApplication.offer_sent_at,
      metadata: updatedApplication.metadata
    }
  }
}
