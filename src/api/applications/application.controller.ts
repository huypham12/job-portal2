...
export class ApplicationController {
  ...
  getOffers = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const result = await this.applicationService.getOffers(profile_id, filters)
  ...
  acceptOffer = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { profile_id } = req.decoded_authorization as TokenPayload
      const { id } = req.params
      const data = req.body as AcceptOfferDTO['body']

      const application = await this.applicationService.acceptOffer(profile_id, id, data)
  ...
  declineOffer = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { profile_id } = req.decoded_authorization as TokenPayload
      const { id } = req.params
      const data = req.body as DeclineOfferDTO['body']

      const application = await this.applicationService.declineOffer(profile_id, id, data)
   ...
   */
  addCandidateFeedback = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { profile_id } = req.decoded_authorization as TokenPayload
      const { id, stageId } = req.params
      const data = req.body as CandidateFeedbackDTO['body']

      const stage = await this.applicationService.addCandidateFeedback(profile_id, id, stageId, data)
