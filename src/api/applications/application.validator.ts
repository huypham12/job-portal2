...
/**
 * Validator for getting offers list
 */
...
/**
 * Validator for candidate feedback on interview stage
 */
const CandidateFeedbackParamsSchema = z.object({
  id: z.string().uuid({ message: 'Application ID must be a valid UUID' }),
  stageId: z.string().uuid({ message: 'Stage ID must be a valid UUID' })
})

const CandidateFeedbackBodySchema = z.object({
  candidate_feedback: z.string().min(1, { message: 'Feedback cannot be empty' })
})

export const CandidateFeedbackSchema = {
  params: CandidateFeedbackParamsSchema,
  body: CandidateFeedbackBodySchema
}

export type CandidateFeedbackDTO = {
  params: z.infer<typeof CandidateFeedbackParamsSchema>
  body: z.infer<typeof CandidateFeedbackBodySchema>
}
