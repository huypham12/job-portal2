...
export const createConnectionInterest = async (recruiter_id: string, data: CreateInterestDTO) => {
  ...
  // If suggested_job_ids provided, verify all jobs exist and belong to recruiter
  ...
  // If suggested_job_ids exists, fetch suggested jobs details
  let suggestedJobs = null
  ...
  await NotificationHelper.notifyConnectionInterestReceived({
    ...
    suggestedJobIds: suggestedJobs ? suggestedJobs.map((j) => j.id) : undefined
  })

  // Return interest with suggested jobs
  return {
    ...interest,
...
export const getConnectionInterests = async (
  ...
  // Fetch suggested jobs for each interest that has suggested_job_ids
